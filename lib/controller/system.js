    ...
    if (self.isDocker) {
        Async.series([
            function (callback) {
                ...
                var dockerImage = 'pufferpanel/' + parent.config.plugin + ':latest';
                if (parent.plugin.settings.docker_image !== undefined) {
                    Logger.verbose(Util.format('Using image from plugin: %s', parent.config.docker_image));
                    dockerImage = parent.plugin.settings.docker_image;
                } else if (parent.config.plugin === 'bungeecord' || parent.config.plugin === 'minecraft-pre') {
                    dockerImage = 'pufferpanel/minecraft:latest';
                }
